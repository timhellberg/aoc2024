				...
				for y2 := y1; y2 < len(grid); y2++ {
					for x2 := 0; x2 < len(row); x2++ {
						...
						if grid[y2][x2] == char {
							...
							higherAntiNode := higherCharPosition + distance
							lowerAntiNode := lowerCharPosition - distance

							if real(lowerAntiNode) >= 0 && real(lowerAntiNode) < float64(len(row)) && imag(lowerAntiNode) >= 0 {
								antinodes[lowerAntiNode] = struct{}{}
							}

							if real(higherAntiNode) >= 0 && real(higherAntiNode) < float64(len(row)) && imag(higherAntiNode) < float64(len(grid)) {
